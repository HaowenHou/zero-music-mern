...
export default function PlaylistAsItem({ playlist, manageMode, onDelete }) {
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`/api/playlist?id=${playlist._id}`);
      if (response.status === 200) {
        onDelete(playlist._id);  // Call the onDelete handler
      } else {
