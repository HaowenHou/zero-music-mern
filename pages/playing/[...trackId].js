...
export default function Playing() {
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(`/api/comments/${trackId}`);
      console.log(response.data);
  ...
  const postComment = async () => {
    try {
      await axios.post(`/api/comments/${trackId}`, { userId, content, timestamp: new Date() });
      setContent('');
      fetchComments();
    } catch (error) {
      console.error(error);
                ...
                <div key={comment._id} className='flex space-x-2'>
                  <div>
                    <Link href={`/profile/${comment.userId._id}`}>
                      <img src={comment.userId.avatar} alt="Avatar" className="w-9 h-9 rounded-full ml-2" />
                    </Link>
                  </div>
